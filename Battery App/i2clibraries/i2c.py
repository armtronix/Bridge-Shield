...
class i2c:
	...
	def read_16bittemp(self, register, flip = False):
		data = self.i2c_device.transaction(
			writing_bytes(self.addr, register),
			reading(self.addr, 2))[0]
		
		if flip:
			bit16 = (((data[0] << 8) | data[1]) >>7) * 0.5

			if bit16 > 125:
					bit16 =(((((data[0] << 8) | data[1]) >>7) * 0.5) -256)
		else:
			bit16 = (data[0] << 8) | data[1]
	...
	def read_s16inttemp(self, register, flip = False):
		s_int = self.read_16bittemp(register, flip)
		return self.twosToInt(s_int, 16)
